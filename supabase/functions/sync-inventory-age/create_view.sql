...
FROM public.fact_inventory_planning f
    LEFT JOIN (
        ...
        SELECT
            account_id, sku, SUM(
                CASE
                    WHEN bucket = '0-60' THEN item_count
                    ELSE 0
                END
            ) AS age_0_60_days, SUM(
                CASE
                    WHEN bucket = '61-90' THEN item_count
                    ELSE 0
                END
            ) AS age_61_90_days, SUM(
                CASE
                    WHEN bucket = '91-180' THEN item_count
                    ELSE 0
                END
            ) AS age_91_180_days, SUM(
                CASE
                    WHEN bucket = '181+' THEN item_count
                    ELSE 0
                END
            ) AS age_181_plus_days
