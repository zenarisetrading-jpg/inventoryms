-- 068_update_metrics_and_refresh_for_saddl_id.sql
-- 1. Add saddl_id to demand_metrics and update PK
ALTER TABLE public.demand_metrics ADD COLUMN IF NOT EXISTS saddl_id TEXT DEFAULT 'none' NOT NULL;
ALTER TABLE public.demand_metrics DROP CONSTRAINT IF EXISTS demand_metrics_pkey CASCADE;
ALTER TABLE public.demand_metrics ADD PRIMARY KEY (sku, country, saddl_id);

-- 2. Add country and saddl_id to allocation_plans and update UNIQUE constraint
ALTER TABLE public.allocation_plans ADD COLUMN IF NOT EXISTS country TEXT DEFAULT 'UAE' NOT NULL;
ALTER TABLE public.allocation_plans ADD COLUMN IF NOT EXISTS saddl_id TEXT DEFAULT 'none' NOT NULL;
ALTER TABLE public.allocation_plans DROP CONSTRAINT IF EXISTS allocation_plans_sku_node_plan_date_key CASCADE;
-- The old constraint name might be allocation_plans_sku_node_plan_date_key
ALTER TABLE public.allocation_plans ADD CONSTRAINT allocation_plans_sku_node_plan_date_country_saddl_id_key UNIQUE (sku, node, plan_date, country, saddl_id);

-- 3. Update refresh_fact_sales_data to loop through all active saddl_ids
...
BEGIN
    ...
    LOOP
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE a.report_date >= today_date - (days_back || ' days')::interval
                  AND UPPER(COALESCE(a.country, 'UAE')) = UPPER(loc.country)
                  AND COALESCE(a.saddl_id, 'none') = loc.saddl_account_id
                GROUP BY 
                    a.report_date,
                    s.asin,
                ...
                WHERE CAST(n.order_timestamp AS DATE) >= today_date - (days_back || ' days')::interval
                  AND (CASE WHEN UPPER(COALESCE(n.src_country, 'AE')) = 'AE' THEN 'UAE' WHEN UPPER(COALESCE(n.src_country, 'AE')) IN ('SA', 'KSA') THEN 'KSA' ELSE UPPER(COALESCE(n.src_country, 'UAE')) END) = UPPER(loc.country)
                  AND COALESCE(n.saddl_id, 'none') = loc.saddl_account_id
                GROUP BY 
                ...
                WHERE m.order_date >= today_date - (days_back || ' days')::interval
                  AND (CASE WHEN UPPER(COALESCE(m.country_code, 'AE')) = 'AE' THEN 'UAE' WHEN UPPER(COALESCE(m.country_code, 'AE')) IN ('SA', 'KSA') THEN 'KSA' ELSE UPPER(COALESCE(m.country_code, 'UAE')) END) = UPPER(loc.country)
                  AND COALESCE(m.saddl_id, 'none') = loc.saddl_account_id
                GROUP BY 
                    m.order_date,
                    s.asin,
        ...
        AND COALESCE(f.country,'') = UPPER(loc.country)
        AND COALESCE(f.saddl_id,'') = loc.saddl_account_id
        AND (
