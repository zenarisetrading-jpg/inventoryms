-- 075_fix_coverage_health_rpc.sql
-- Fixes the get_coverage_health RPC which was accidentally returning distribution data instead of median coverage days.
...
BEGIN
    ...
    FROM (
        ...
        LEFT JOIN sku_master sm ON f.sku = sm.sku AND UPPER(COALESCE(sm.country, 'UAE')) = CASE 
                WHEN p_saddl_id IN ('aurio_ksa', 's2c_ksa_test', 's2c_test') THEN 'KSA'
                ELSE 'UAE'
            END
...
GRANT EXECUTE ON FUNCTION public.get_coverage_health(TEXT[], TEXT[], TEXT[], TEXT) TO anon;
GRANT EXECUTE ON FUNCTION public.get_coverage_health(TEXT[], TEXT[], TEXT[], TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_coverage_health(TEXT[], TEXT[], TEXT[], TEXT) TO service_role;
