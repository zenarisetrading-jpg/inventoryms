-- Migration: 016_minutes_inventory_sales.sql
-- Add Minutes support for inventory and sales

-- 1. Update constraints on sales_snapshot and inventory_snapshot
ALTER TABLE sales_snapshot DROP CONSTRAINT IF EXISTS sales_snapshot_channel_check;
ALTER TABLE sales_snapshot ADD CONSTRAINT sales_snapshot_channel_check CHECK (channel IN ('amazon', 'noon', 'noon_minutes'));

ALTER TABLE inventory_snapshot DROP CONSTRAINT IF EXISTS inventory_snapshot_node_check;
ALTER TABLE inventory_snapshot ADD CONSTRAINT inventory_snapshot_node_check CHECK (node IN ('amazon_fba', 'noon_fbn', 'locad_warehouse', 'Minutes'));

-- 2. Add columns to fact_inventory_planning and demand_metrics
ALTER TABLE fact_inventory_planning ADD COLUMN IF NOT EXISTS minutes_units NUMERIC DEFAULT 0;
ALTER TABLE fact_inventory_planning ADD COLUMN IF NOT EXISTS minutes_sv NUMERIC DEFAULT 0;
ALTER TABLE demand_metrics ADD COLUMN IF NOT EXISTS minutes_sv NUMERIC DEFAULT 0;

-- 3. Update the refresh function to include Minutes logic
...
BEGIN
    ...
    final_calc AS (
        SELECT *,
            ...
            GREATEST(
                (amazon_required_30 + noon_required_30 + minutes_required_30) 
                ...
                ELSE GREATEST(
                    ...
                    CEIL((blended_sv * (
                        CASE 
                            ...
                            ELSE 30
                        END
                    )) / NULLIF(units_per_box, 0)) * units_per_box
                )
