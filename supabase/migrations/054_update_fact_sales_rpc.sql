-- =========================================================
-- 054_update_fact_sales_rpc.sql
-- Add p_country filter to all fact_sales related RPCs
-- =========================================================

-- DROP ALL OLD SIGNATURES TO PREVENT AMBIGUITY
...
DROP FUNCTION IF EXISTS public.get_subcategory_performance(int, text[], text[], text[]);
...
BEGIN
    ...
    range_stats AS (
        ...
        LEFT JOIN sku_master sm ON f.sku = sm.sku AND sm.country = p_country
        GROUP BY dr.label
    ),
    calculated_stats AS (
        SELECT 
...
-- 4. get_mtd_sales
CREATE OR REPLACE FUNCTION public.get_mtd_sales(p_country TEXT DEFAULT 'UAE')
RETURNS JSONB AS $$
DECLARE
    result JSONB;
    today_date DATE;
...
-- 5. get_today_sales
CREATE OR REPLACE FUNCTION public.get_today_sales(p_country TEXT DEFAULT 'UAE')
RETURNS JSONB AS $$
DECLARE
    result JSONB;
    today_date DATE;
