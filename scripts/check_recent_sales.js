...
const URL = 'https://eiezhzlpirdiqsotvogx.supabase.co/rest/v1/sales_snapshot?sku=eq.6PCGREYNEW&order=date.desc&limit=10'

async function checkSales() {
    const res = await fetch(URL, { headers: { 'apikey': ANON_KEY, 'Authorization': `Bearer ${ANON_KEY}` } })
    const data = await res.json()
    console.log('Recent Sales for 6PCGREYNEW:', data)
}
