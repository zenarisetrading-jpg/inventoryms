...
export function SKUBulkUploadModal({ onClose, onSuccess }: SKUBulkUploadModalProps) {
  ...
  const downloadTemplate = () => {
    const header = 'sku,asin,fnsku,name,category,product_category,sub_category,moq,lead_time_days,cogs,shipping_cost,landing_cost,units_per_box,dimensions,weight_kg,cbm,is_active,amazon_active,noon_active,minutes_active,saddl_id'
    const example = 'SKU-001,B00EXAMP1,X00EXAMP2,Sample Item,A,Drinkware,Tumblers,50,30,12.5,2.5,15.0,10,10x10x10,1.5,0.01,true,true,true,true,s2c_uae_test'
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-lg border border-zinc-200">
        ...
        <div className="px-5 py-5 space-y-4">
          ...
          <div className="bg-zinc-50 border border-zinc-200 rounded-md p-3 text-xs text-zinc-600 space-y-1">
            ...
            <p className="overflow-x-auto whitespace-nowrap pb-1 font-mono text-zinc-500">sku, asin, fnsku, name, category, product_category, sub_category, moq, lead_time_days, cogs, shipping_cost, landing_cost, units_per_box, dimensions, weight_kg, cbm, is_active, amazon_active, noon_active, minutes_active, saddl_id</p>
            <ul className="mt-1.5 space-y-0.5 list-disc list-inside text-zinc-500">
              <li><span className="font-mono text-zinc-700">sku</span> is the unique identifier per location.</li>
